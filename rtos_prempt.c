...
bool createThread(_fn fn, char name[], int priority) {
	...
	if (taskCount <= MAX_TASKS) {
		...
		if (!found) {
			...
			//preload stack to look like the task had run before
...
void rtosStart() {
	...
	// code to initialize the SP with tcb[task_current].sp;
	void *psp1;
	psp1 = tcb[task].sp;
...
void pendsvISR() {
	...
	__asm(" MOV R0,LR ");
	LR_VALUE = returnlr(); // Meachanism to save the current LR value.
...
bool iscommand2(char fieldname[10]) {
	bool answer;
	answer = (strcmp(fieldname, &check[fieldposition[1]]) == 0);
	return answer;
}
...
void shell() {
	// processing for the shell commands here
	int i = 0;
	while (1) {
		i = 0;
		charcount = 0;
		...
		while (i < 32) {
			input = getcUart0();
			if ((input > 96 && input < 123) | (input > 47 && input < 58)
					| (input == 32) | (input > 32 && input < 48)
					| (input > 57 && input < 65) | (input > 90 && input < 97)) {
				i++;
				check[i - 1] = input;
				charcount++;
				output[i - 1] = input - 32;
			...
			if ((input > 32 && input < 48) | (input > 57 && input < 65)
					| (input > 90 && input < 97) | (input == 32)) {
		...
		while (cnt1 < 32) {
			...
			if ((type1[cnt1] == 's' && type1[cnt1 + 1] == 'a')
					|| (type1[cnt1] == 's' && type1[cnt1 + 1] == 'n')) {
